...
struct rr_ui_element *rr_ui_petal_tooltip_init(uint8_t id, uint8_t rarity)
{
        ...
        rr_ui_container_add_element(
            this, rr_ui_set_justify(
                      rr_ui_h_container_init(
                          rr_ui_container_init(), 0, 0,
                          rr_ui_text_init(
                              "By Toutatis! The sky has fallen on our heads!",
                              12, 0xeaddcaff),
                          rr_ui_text_init(extra, 12, 0xfffffff), NULL),
                      -1, 0));
    }
        ...
        rr_ui_container_add_element(
            this, rr_ui_set_justify(
                      rr_ui_h_container_init(
                          ...
                          rr_ui_text_init(
                              RR_RARITY_NAMES[rarity >= 2 ? rarity - 1 : 0], 12,
                              RR_RARITY_COLORS[rarity >= 2 ? rarity - 1 : 0]),
    ...
    {
        float basedmg = 2.0;
        float raritymulti = 3.0 + (rarity * 1.0);
        float damage = basedmg + (rarity * raritymulti);

        char *extra = malloc((sizeof *extra) * 16);
        sprintf(extra, "%s", rr_sprintf(fmt, damage));
        char *dmgl = "Self Damage: ";
        char *rangel = "Range: ";
        rr_ui_container_add_element(
            this,
            rr_ui_set_justify(
                rr_ui_v_container_init(
                    ...
                    rr_ui_set_justify(
                        rr_ui_h_container_init(
                            rr_ui_container_init(), 0, 0,
                            rr_ui_text_init(dmgl, 12, 0x9e2118),
                            rr_ui_text_init(extra, 12, 0xffffffff), NULL),
                        -1, 0),
                    rr_ui_set_justify(
                        rr_ui_h_container_init(
                            rr_ui_container_init(), 0, 0,
                            rr_ui_text_init(rangel, 12, 0xffbf29c2),
                            rr_ui_text_init(rr_sprintf(fmt, 100 + 75 * rarity),
                                            12, 0xffffffff),
                            NULL),
                        -1, 0),
                    NULL),
    ...
    else if (id == rr_petal_id_bone)
    {
        char *extra = malloc((sizeof *extra) * 8);
        sprintf(extra, "%s",
                rr_sprintf(fmt, RR_PETAL_RARITY_SCALE[rarity].bone_reduction));
        rr_ui_container_add_element(
            this, rr_ui_set_justify(
                      rr_ui_h_container_init(
                          rr_ui_container_init(), 0, 0,
