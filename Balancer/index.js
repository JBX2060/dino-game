...
let RR_DROP_RARITY_COEFFICIENTS = [0, 1, 8, 15, 40, 150, 500, 2500];
let RR_MOB_RARITY_COEFFICIENTS = [3, 3, 3, 3, 3, 3, 15];

function init_rarity_coefficients() {
  for (let i = 0; i < rr_rarity_id_max; ++i) {
    RR_DROP_RARITY_COEFFICIENTS[i + 1] = parseInt(
      document.getElementById("d" + i).value
    );
    RR_MOB_RARITY_COEFFICIENTS[i] = parseInt(
  ...
  for (let a = 1; a < rr_rarity_id_max; ++a) {
    sum += RR_DROP_RARITY_COEFFICIENTS[a + 1] =
      RR_DROP_RARITY_COEFFICIENTS[a] / RR_DROP_RARITY_COEFFICIENTS[a + 1];
    RR_MOB_RARITY_COEFFICIENTS[a] *= RR_MOB_RARITY_COEFFICIENTS[a - 1];
  }
  for (let a = 1; a <= rr_rarity_id_max; ++a)
    RR_DROP_RARITY_COEFFICIENTS[a] =
      RR_DROP_RARITY_COEFFICIENTS[a] / sum + RR_DROP_RARITY_COEFFICIENTS[a - 1];
...
function loop() {
  ...
  for (let mob = 0; mob < rr_rarity_id_max; ++mob) {
    ...
    const text =
      (100 * Math.pow(1 - seed, RR_MOB_RARITY_COEFFICIENTS[mob])).toFixed(2) +
      "%";
    ...
    for (let drop = 0; drop <= cap; ++drop) {
      ...
      const text =
        (
          100 *
          (Math.pow(1 - (1 - end) * seed, RR_MOB_RARITY_COEFFICIENTS[mob]) -
            Math.pow(1 - (1 - start) * seed, RR_MOB_RARITY_COEFFICIENTS[mob]))
        ).toFixed(2) + "%";
