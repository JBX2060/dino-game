                                   ...
                                   EntityIdx from, float v)
{
    ...
    {
        struct rr_component_mob *from_mob =
            rr_simulation_get_mob(simulation, from);
        ...
        if (rr_simulation_has_mob(simulation, this->parent_id))
        {
            ...
            if (mob->id == rr_mob_id_edmontosaurus &&
                from_mob->player_spawned &&
                (from_mob->rarity < mob->rarity ||
                 from_mob->rarity - mob->rarity < 2))
                return;
        }
    }
