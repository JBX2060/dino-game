...
{
    return rr_frand() > 0.5 ? rr_mob_id_ornithomimus
...
static void set_spawn_zones()
{
    ...
    for (uint64_t i = 0; i < ROT_COUNT; i++)
    {
        ...
        set_special_zone(rr_biome_id_swamp, z.spawn_func, z.x, z.y, z.w, z.h);
    }
}
...
void rr_biome_hell_creek_init(struct rr_simulation *this)
{
    memset(this, 0, sizeof *this);
    EntityIdx id = rr_simulation_alloc_entity(this);
    struct rr_component_arena *hell_creek = rr_simulation_add_arena(this, id);
    hell_creek->biome = rr_biome_id_hell_creek;
    rr_component_arena_spatial_hash_init(hell_creek, this);
    set_respawn_zone(hell_creek, SPAWN_ZONE_X, SPAWN_ZONE_Y);
}

void rr_biome_swamp_init(struct rr_simulation *this)
{
    memset(this, 0, sizeof *this);
    EntityIdx id = rr_simulation_alloc_entity(this);
    struct rr_component_arena *swamp = rr_simulation_add_arena(this, id);
    swamp->biome = rr_biome_id_swamp;
    rr_component_arena_spatial_hash_init(swamp, this);
    set_respawn_zone(swamp, SPAWN_ZONE_X, SPAWN_ZONE_Y);
}

void rr_biome_garden_init(struct rr_simulation *this)
{
    memset(this, 0, sizeof *this);
    EntityIdx id = rr_simulation_alloc_entity(this);
    struct rr_component_arena *garden = rr_simulation_add_arena(this, id);
    garden->biome = rr_biome_id_grass;
    rr_component_arena_spatial_hash_init(garden, this);
    set_respawn_zone(garden, SPAWN_ZONE_X, SPAWN_ZONE_Y);
}
...
{
    if (entity == rr_mob_id_portal_swamp)
        return;
    struct rr_simulation *this = _simulation;
    struct rr_component_physical *physical =
        rr_simulation_get_physical(this, entity);
            ...
            ->player_count == 0)
    {
        ...
        {
            if (mob->id == rr_mob_id_portal_swamp)
                return;
...
static void tick_maze_swamp(struct rr_simulation *this)
{
    struct rr_component_arena *arena = rr_simulation_get_arena(this, 1);
    for (uint32_t i = 0; i < arena->maze->maze_dim * arena->maze->maze_dim; ++i)
        arena->maze->maze[i].local_difficulty =
