...
#include <Server/ProfanityFilter.h>
...
#undef XX
    ...
    // #ifndef RIVET_BUILD
    // RR_GLOBAL_BIOME = rr_biome_id_centralia;
    // #endif
    rr_static_data_init();
                            ...
                            size_t size)
{
    ...
    case LWS_CALLBACK_RECEIVE:
    {
        ...
        if (!client->received_first_packet)
        {
            ...
            // #endif
            // if (strcmp(client->rivet_account.uuid,
            //            "eec8eb1a-e926-4fa4-8424-64a90b4s72r5") == 0 ||
            //     strcmp(client->rivet_account.uuid,
            //            "8c41294c-a732-4415-ac78-8b182cecde07") == 0 ||
            //     strcmp(client->rivet_account.uuid,
            //            "ec44e6f3-f2a1-4160-a61a-69761718g16a") == 0)
            //     client->dev = 1;
                ...
                if (client->dev)
                {
                    ...
                    if (squad->private)
                        for (uint32_t i = 0; i < RR_MAX_CLIENT_COUNT; ++i)
                            rr_bitset_unset(
                                this->clients[i].joined_squad_before,
                                client->squad);
                }
        ...
        case rr_serverbound_chat:
        {
            ...
            uint64_t current_time = (uint64_t)time_now.tv_sec * 1000 +
                                    (uint64_t)time_now.tv_usec / 1000;
            if (current_time - client->last_message_time < 2000)
                break;
            ...
            char *trimmed_message = rr_trim_string(message);
            if (trimmed_message[0] == 0)
                break;
            if (!rr_validate_user_string(trimmed_message))
            {
                printf("blocked chat: %s: %s\n", name, trimmed_message);
                break;
            }
            if (contains_profanity(trimmed_message))
            {
                printf("blocked profanity chat: %s: %s\n", name,
                       trimmed_message);
                break;
            }
            ...
            strcpy(animation->message, trimmed_message);
        ...
        //     // uint8_t id = proto_bug_read_uint8(&encoder, "id");
        //     // uint8_t rarity = proto_bug_read_uint8(&encoder, "rarity");
        //     // uint8_t id = rr_mob_id_portal_hell_creek; // rand() %
        //     // rr_mob_id_max; uint8_t rarity = rr_rarity_id_max - 5;

        //     // for (uint8_t i = 0; i < 1; i++)
        //     // {
        //     //     EntityIdx e = rr_simulation_alloc_mob(
        //     //         &this->simulation, client->player_info->arena,
        //     //         client->player_info->camera_x + (rr_frand() * 2000 -
        //     //         1000), client->player_info->camera_y + (rr_frand() * 2000
        //     //         - 1000), id, rarity, rr_simulation_team_id_mobs);
        //     //     struct rr_component_mob *mob =
        //     //         rr_simulation_get_mob(&this->simulation, e);
        //     //     mob->no_drop = 1;
        //     // }
        //     // struct rr_component_arena *arena = rr_simulation_get_arena(
        //     //     &this->simulation, client->player_info->arena);
        //     // for (uint8_t i = 0; i < 1; ++i)
        ...
        //     //         float x = rr_fclamp(
        //     //             client->player_info
        //     //                 ->camera_x /* + (rr_frand() * 2000 - 1000)*/,
        //     //             0, arena->maze->grid_size * arena->maze->maze_dim);
        //     //         float y = rr_fclamp(
        //     //             client->player_info
        //     //                 ->camera_y /* + (rr_frand() * 2000 - 1000)*/,
        //     //             0, arena->maze->grid_size * arena->maze->maze_dim);
        ...
        //     //         EntityIdx e = rr_simulation_alloc_mob(
        //     //             &this->simulation, client->player_info->arena, x, y,
        ...
        //     // uint8_t rarity = proto_bug_read_uint8(&encoder, "rarity");
        //     // uint8_t id =
        //     // rr_mob_id_portal_garden; // rand() % (rr_mob_id_edmontosaurus +
        //     // 1);
        ...
        //             EntityIdx e = rr_simulation_alloc_mob(
        //                 &this->simulation, client->player_info->arena, x, y, id,
        //                 rarity, rr_simulation_team_id_mobs);
        //             struct rr_component_mob *mob =
        //                 rr_simulation_get_mob(&this->simulation, e);
        //             mob->no_drop = 1;
        //             break;
...
{
    if (!ws) {
        lwsl_err("WebSocket pointer is null\n");
        return -1;
    }

    struct lws_context *context = lws_get_context(ws);
    if (!context) {
        lwsl_err("Context retrieval failed\n");
        return -1;
    }

    struct rr_server *this = (struct rr_server *)lws_context_user(context);
    if (!this) {
        lwsl_err("Failed to get server instance from context\n");
        return -1;
    }

    int lock_result = pthread_mutex_lock(&mutex);
    if (lock_result != 0) {
        lwsl_err("Mutex lock failed with error: %d\n", lock_result);
        return -1;
    }
    ...
    case LWS_CALLBACK_RECEIVE:
    case LWS_CALLBACK_CLOSED:
        close = handle_lws_event(this, ws, reason, packet, size);
        break;
    default:
        break;
    }

    lock_result = pthread_mutex_unlock(&mutex);
    if (lock_result != 0) {
        lwsl_err("Mutex unlock failed with error: %d\n", lock_result);
        return -1;
    }
...
#endif
        client_info.port =
            getenv("MASTERPORT") ? atoi(getenv("MASTERPORT")) : 55554;
