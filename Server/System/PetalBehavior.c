    ...
    struct rr_petal_data const *petal_data)
{
    ...
    if (petal->effect_delay == 0)
    {
        ...
        case rr_petal_id_salt:
        {
            if ((player_info->input & 3) == 0)
                break;
            system_petal_detach(simulation, petal, player_info, outer_pos,
                                inner_pos, petal_data);
            petal->effect_delay = 200;
            physical->friction = 0.2;
            break;
        }
    ...
    for (uint64_t outer = 0; outer < player_info->slot_count; ++outer)
    {
        ...
        else if (data->id == rr_petal_id_feather)
        {
            physical->acceleration_scale +=
                (0.05 * slot->rarity) * feather_diminish_factor;
            feather_diminish_factor *= 0;
        }
        ...
        // {
        //     float basedmg = 0.5;
        //     float raritymulti = 0.75 + (slot->rarity * 0.25);
        //     float damage = basedmg + (slot->rarity * raritymulti);
                                                    ...
                                                    void *simulation)
{
    ...
    for (uint64_t outer = 0; outer < player_info->slot_count; ++outer)
    {
        ...
        for (uint64_t inner = 0; inner < slot->count; ++inner)
        {
            ...
            {
                struct rr_component_health *health =
                    rr_simulation_get_health(simulation, p_petal->entity_hash);
                float hp = rr_fclamp(
                ...
                if (data->id == rr_petal_id_egg)
                {
                    ...
                    {
                        m_id = rr_mob_id_trex;
                        m_rar = petal->rarity >= 2 ? petal->rarity - 1 : 0;
                    }
