                                     ...
                                     EntityIdx arena_id, EntityIdx entity)
{
    ...
    rr_component_health_set_max_health(
        health,
        (player_info->dev ? 1000 : 1) * 100 *
            pow(1.0256, player_info->level > 120 ? 120 : player_info->level));
    ...
    // uint32_t oldArena = player_info->arena;
    uint32_t X = 0;
    uint32_t Y = 20;
    /*EntityIdx portal = rr_simulation_alloc_mob(
        this, rr_biome_id_hell_creek, X * arena->maze->grid_size, Y *
    arena->maze->grid_size, rr_mob_id_portal_swamp, rr_rarity_id_epic,
    rr_simulation_team_id_mobs); struct rr_component_mob *mob =
        rr_simulation_get_mob(this, portal);
    mob->no_drop = 1;*/

    // player_info->arena = oldArena;
                                  ...
                                  enum rr_simulation_team_id team_id)
{
    ...
    rr_component_relations_set_team(relations, team_id);
    rr_component_relations_update_root_owner(this, relations);
    if (mob_id == rr_mob_id_portal_centralia)
    {
        struct rr_component_arena *arena =
            rr_simulation_add_arena(this, entity);
        rr_component_arena_set_biome(arena, rr_biome_id_centralia);
        rr_component_arena_spatial_hash_init(arena, this);
        set_respawn_zone(arena, 20, 20);
    }
    else if (mob_id == rr_mob_id_portal_hell_creek)
    {
        struct rr_component_arena *arena =
            rr_simulation_add_arena(this, entity);
        rr_component_arena_set_biome(arena, rr_biome_id_hell_creek);
        rr_component_arena_spatial_hash_init(arena, this);
        //set_respawn_zone(arena, 6, 13);
        // Keep arena alive
        ++arena->mob_count;
        // int64_t X = 20;
        // int64_t Y = 0.5;
        // EntityIdx bee_entity = rr_simulation_alloc_mob(
        //     this, entity, (X)*arena->maze->grid_size,
        //     (Y)*arena->maze->grid_size, rr_mob_id_portal_hell_creek,
        //     rarity_id, team_id);
    }
    else if (mob_id == rr_mob_id_portal_swamp)
    {
        struct rr_component_arena *arena =
            rr_simulation_add_arena(this, entity);
        rr_component_arena_set_biome(arena, rr_biome_id_swamp);
        rr_component_arena_spatial_hash_init(arena, this);
        //set_respawn_zone(arena, 1, 33);
        ++arena->mob_count;
        // int64_t X = 20;
        // int64_t Y = 0;
        // EntityIdx bee_entity = rr_simulation_alloc_mob(
    ...
    else if (mob_id == rr_mob_id_portal_garden)
    {
        struct rr_component_arena *arena =
            rr_simulation_add_arena(this, entity);
        rr_component_arena_set_biome(arena, rr_biome_id_grass);
        rr_component_arena_spatial_hash_init(arena, this);
        //set_respawn_zone(arena, 1, 33);
        ++arena->mob_count;
        // int64_t X = 20;
